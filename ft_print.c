...
void 	ft_handlerightleft(f_list *p, int size) /////probably don't need it at all
{
	if (p->f_oct)
		ft_handlehash(p, size);
	else
	{
		...
		(p->f_plus && p->pr ) ? ft_putcratch('+', '0', p->pr - size) : 0;
		(p->pr > size && !p->f_plus) ? ft_putnchar('0', p->pr - size) : 0;
		/*(p->w > size && p->f_plus && !p->pr) ? ft_putcratch('+', '0', p->w - size) : 0;
		(p->w > size && !p->f_plus && !p->f_minus) ? ft_putnchar('0', p->w - size) : 0;
		(p->w > p->pr && p->pr > size && !p->f_plus) ? ft_putcratch(' ', '0', p->pr - size) : 0;
		(p->pr > p->w  && p->w > size && !p->f_plus ) ? ft_putnchar('0', p->pr - size) : 0;
		(p->w > p->pr  && p->w > size) ? ft_puttriplecratch('+','0',p->w - p->pr - 1, p->pr - size) : 0;
		(p->w < p->pr  && p->w > size && !p->f_minus) ? ft_putcratch('+','0', p->pr - size) : 0;
		(p->f_plus && !p->w) ? ft_putchar('+') : 0;
		(p->pr > size && !p->f_plus && !p->f_minus) ? ft_putnchar('0', p->pr - size) : 0;*/
...
void	ft_handlehash(f_list *p, int size)
{
		...
		(p->w && !p->pr && !p->f_zero && p->ispos != 0) ? ft_crutchforhashv2(p, ' ', p->w - size) : 0;//
...
{
	//printf("--------------->>>>>>>>>>>. %d\n", p->ispos);	
	if (p->ispos == 2 && p->conversion != 'x' && p->conversion != 'X' && p->conversion != 'u' && p->conversion != 'U')
	{
		ft_handleneg(p, size);
	}
	else
